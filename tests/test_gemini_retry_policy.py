...
def test_default_thinking_budget_matches_across_sync_and_async(monkeypatch):
    ...
    asyncio.run(
        query_gemini_async(_AsyncClient(), "gemini-2.5-flash", "msg", "sys", [], None)
    )
...
def test_sync_llm_client_does_not_retry_structured_output_error(monkeypatch):
    ...
    monkeypatch.setattr(
        llm_module.time, "sleep", lambda seconds: sleeps.append(seconds)
    )
...
def test_sync_public_batch_propagates_structured_output_error(monkeypatch, method_name):
