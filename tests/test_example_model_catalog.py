...
# The interactive sandbox monkeypatches both clients and never sends these
# identifiers to a provider. Keep this exemption scoped to that test fixture.
INTERACTIVE_SANDBOX_PREFIX = "examples/interactive_sandbox/"
INTERACTIVE_SANDBOX_MOCK_PREFIX = "mock-"
...
def test_example_models_resolve_with_finite_pricing(reference: ModelReference) -> None:
    if reference.source.startswith(
        INTERACTIVE_SANDBOX_PREFIX
    ) and reference.model_name.startswith(INTERACTIVE_SANDBOX_MOCK_PREFIX):
        return
